            ...
            makeNodeIcoStyle: function (setting, node) {
                ...
                if (!node.isAjaxing) {
                 ...
                 // var icon = (isParent && node.iconOpen && node.iconClose) ? (node.open ? node.iconOpen : node.iconClose) : node[setting.data.key.icon];
                    var icon = -1;
                    if(setting.view.remoteIcon) {
                    	icon = (isParent && node.iconOpen && node.iconClose) ? (node.open ? node.iconOpen : node.iconClose) : node[setting.data.key.icon];
                    } else {
                    	icon = node.open ? node.iconOpen : node.iconClose;
                    }
