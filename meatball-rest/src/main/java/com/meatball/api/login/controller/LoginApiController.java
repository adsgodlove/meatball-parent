...
public class LoginApiController {
	...
	/**
	 * @Title: validate 
	 * @Description: TODO(获取微信授权，并返回token) 
	 * @param user
	 * @RequestBody 必须指定参数获取位置，否则无法获取到参数
	 * @return boolean    返回类型
	 */
	@ApiOperation(value = "获取微信授权，并返回token", notes = "验证微信请求")
	@ApiResponses({ 
		@ApiResponse(code = 200, message = "权限验证成功", response = String.class), 
		@ApiResponse(code = 201, message = "请求成功并且服务器创建了新的资源", response = Void.class), 
		@ApiResponse(code = 401, message = "用户名或密码错", response = Void.class), 
		@ApiResponse(code = 403, message = "权限认证失败", response = Void.class),
		@ApiResponse(code = 404, message = "请求的资源不存在", response = Void.class)
	...
	@OperateLog("微信请求")
	public ResultMsg wxrequest(@RequestBody String code) {
		return service.getWeixinRequestInfo(code);
	}
